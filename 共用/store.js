...
(function (global) {
  ...
  var SEED_ANN = [
    {id:1,title:"端午連假公告：6/20–6/22 休假",cat:"重要",pinned:true,date:"2026-06-18",by:"管理部",
     body:"端午連假 6/20(五)~6/22(日) 共三天，6/23(一) 正常上班。連假期間大樓門禁啟用，如需加班請事先申請。",att:[{name:"端午活動海報.jpg",type:"image/jpeg",data:"共用/ann/duanwu.jpg"}]},
    ...
    {id:3,title:"七月份員工健檢開始報名",cat:"活動",pinned:false,date:"2026-06-16",by:"人資部",
     body:"40 歲以上同仁免費健檢，請於 6/30 前至人資系統報名，名額有限額滿為止。",att:[{name:"健檢宣傳海報.jpg",type:"image/jpeg",data:"共用/ann/health.jpg"},{name:"健檢項目說明.txt",type:"text/plain",data:"共用/ann/健檢項目說明.txt"}]},
    {id:4,title:"差旅費報銷新制上路",cat:"一般",pinned:false,date:"2026-06-12",by:"財務部",
     body:"自 7/1 起差旅費改為線上報銷，紙本單據需掃描上傳，核銷週期縮短為 5 個工作天。",att:[{name:"報銷新制說明.txt",type:"text/plain",data:"共用/ann/報銷新制說明.txt"}]},
    {id:5,title:"新版識別證換發",cat:"一般",pinned:false,date:"2026-06-10",by:"總務部",
     body:"舊識別證 7/15 停用，請各部門彙整至總務部換發，遺失補發酌收工本費。",att:[{name:"新識別證樣式.jpg",type:"image/jpeg",data:"共用/ann/idcard.jpg"}]},
  ...
  function seedNotifs(){
    ...
    return [
      {id:1, to:"王小明",   mod:"ticket",  text:"你的工單（人體工學椅）被退回，需要你補充資料。",            link:"工單通/index.html#/t/k4",  at:t-18*H, read:false},
      {id:2, to:"王小明",   mod:"signoff", text:"你的簽呈（3F 辦公室空間規劃）被退回重擬。",                  link:"電子簽呈/index.html#/d/s3", at:t-45*H, read:false},
      {id:3, to:"周文彬",   mod:"ticket",  text:"王小明 送出主管簽核：南區客戶拜訪差旅預支，待你核准。",      link:"工單通/index.html#/t/k11", at:t-5*H,  read:false},
      {id:4, to:"周文彬",   mod:"signoff", text:"王小明 送簽：2026 年度教育訓練計畫，待你核定。",            link:"電子簽呈/index.html#/d/s1", at:t-20*H, read:false},
      {id:5, to:"林淑芬",   mod:"signoff", text:"資安政策更新（密碼與 VPN 規範）已完成部分會簽，待你最終核定。", link:"電子簽呈/index.html#/d/s4", at:t-1*H,  read:false},
      {id:6, to:"系統管理員", mod:"ticket",  text:"陳大華 送出簽核：申請正式機資料庫只讀權限，待你核准。",      link:"工單通/index.html#/t/k3",  at:t-12*H, read:false}
    ];
  }
  ...
  function seedCards(){ return [
    {id:1, name:"陳建國", company:"宏達科技", title:"採購經理",   phone:"02-2345-6789", email:"jianguo@hodatech.com", address:"台北市內湖區瑞光路 88 號", img:"../共用/att/card1.jpg", createdAt:"2026-06-10"},
    {id:2, name:"林雅婷", company:"全球物流", title:"業務總監",   phone:"0912-345-678", email:"yating@globallog.com",  address:"新北市新莊區中正路 100 號", img:"../共用/att/card2.jpg", createdAt:"2026-06-12"},
  ...
  /* ================= 個人檔案 profile =================
     ...
     photo：使用者上傳為 base64 dataURL；種子為 共用/avatars/ 下的檔名。 */
  var PROFILE_KEY = "ehub_profiles_v1";
  var AVATAR_BASE = "共用/avatars/"; // 首頁預設；工具可設 window.EHUB_AVATAR_BASE 覆寫
  var PHOTOS = {
  ...
  function readFiles(fileList, cb){
    ...
    function push(o){ out.push(o); if(out.length===n) cb(out); }
  ...
  global.EHub = {
    readFiles:readFiles, fmtBytes:fmtBytes,
